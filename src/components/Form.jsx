...
const Form = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('ЭТО КУКИ КОТОРЫЕ Я ПОЛУЧАЮ ПЕРЕД ОТПРАВКОЙ:', {
        accessToken,
        refreshToken
    });
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.status === 'ok') {
                console.log('Рекомендации получены:', data.data);
                setRecommendations(data.data);
                setIsModalOpen(true);
            } else {
                console.log('Ошибка при обработке данных от сервера.');
    ...
    <div className="container mx-auto p-6">
      <form onSubmit={handleSubmit} className="bg-white p-8 shadow-xl rounded-lg w-full max-w-xl ml-7">
        ...
        <fieldset className="space-y-4 mb-6">
          ...
          <button
            type="submit"
            className="mt-4 bg-blue-500 text-white px-6 py-2 rounded-lg hover:bg-blue-600 focus:ring-2 focus:ring-blue-400"
          >
